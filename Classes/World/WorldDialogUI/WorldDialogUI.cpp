#include "WorldDialogUI.h"
#include "Common/Common.h"

USING_NS_CC;

WorldDialogUI::WorldDialogUI()
...
bool WorldDialogUI::load( const std::string& backgroundTexture )
{
	...
	if ( !m_sprite )
	{
		CCLOG("[WorldDialogUI][load]: loading '%s' failed", backgroundTexture.c_str());
		return false;
	}
...
void WorldDialogUI::addPage(const std::string& text )
{
	CCLOG("[WorldDialogUI][addPage]: Adding page!");
	m_otherPages.push(text);
}


void WorldDialogUI::init(const std::string& text) {
	CCLOG("[WorldDialogUI][init]: Init!");
	m_bIsDone = false;
...
void WorldDialogUI::update( float dt ) { 
	if ( getIsDone() ) 
		return; 
...
void WorldDialogUI::processTouchBegan() { 
	if ( getIsDone() ) 
		return; 

	CCLOG("[WorldDialogUI][processTouchBegan]: processTouchBegan!");
...
void WorldDialogUI::hideAllLetters()
{
	CCObject* it = NULL;
...
void WorldDialogUI::showAllLetters()
{
	CCObject* it = NULL;
...
void WorldDialogUI::prepareText(const std::string& text)
{
